...
fn strict_product_quality_expectation() -> SceneRecipeQualityExpectationV1 {
    SceneRecipeQualityExpectationV1 {
        ...
        contrast: Some(crate::SceneRecipeQualityContrastV1 {
            min_luminance_range: None,
            min_sobel_energy: None,
            min_subject_luminance_range: Some(0.12),
        }),
        noise: None,
        text: None,
        line: None,
...
fn decode_png_rgba8_file(path: &Path) -> DecodedPng {
    let bytes = std::fs::read(path)
        .unwrap_or_else(|err| panic!("PNG fixture reads at {}: {err}", path.display()));
