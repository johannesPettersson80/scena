...
pub(super) struct MaterialUniformUpload {
    ...
    /// KHR_materials_iridescence scalar lanes.
    /// .x = iridescenceFactor
    /// .y = iridescenceIor
    ...
    /// KHR_materials_transmission / volume scalar lanes.
    /// .x = transmissionFactor
    /// .y = KHR_materials_ior.ior for refraction
...
impl MaterialUniformUpload {
    ...
    pub(super) fn encode(self) -> [u8; MATERIAL_UNIFORM_BYTE_LEN as usize] {
        ...
        // iridescence_factors follows at offset 160.
        for (index, value) in self.iridescence_factors.into_iter().enumerate() {
            let byte_offset = 160 + index * 4;
            bytes[byte_offset..byte_offset + 4].copy_from_slice(&value.to_ne_bytes());
        }
        // dispersion_factors follows at offset 176.
        for (index, value) in self.dispersion_factors.into_iter().enumerate() {
            let byte_offset = 176 + index * 4;
            bytes[byte_offset..byte_offset + 4].copy_from_slice(&value.to_ne_bytes());
        }
        // transmission_factors follows at offset 192.
        for (index, value) in self.transmission_factors.into_iter().enumerate() {
