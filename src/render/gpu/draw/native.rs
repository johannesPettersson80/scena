...
impl GpuDeviceState {
    ...
    pub(in crate::render) fn render_to_frame(
        ...
        let surface_format = self.surface.as_ref().map(|surface| surface.config.format);
        let scene_format = native_scene_color_format(post_enabled, surface_format, readback);
        if post_enabled
            && !resources
                .post
                .as_ref()
        ...
        self.queue.write_buffer(
            ...
            0,
            &encode_uniform(scene_color_management),
        );
        if let (Some(uniform), Some(color_management)) = (
            resources.surface_output_uniform.as_ref(),
            surface_color_management,
        ) {
            self.queue
                .write_buffer(uniform, 0, &encode_uniform(color_management));
        }
        if resources
            .depth_prepass
            .as_ref()
        ...
        if scene_target_plan != NativeSceneTargetPlan::DirectSurface {
            ...
            let (final_view, final_pipelines, base_label) = if post_enabled {
                ...
                (
                    &resources.view,
                    super::super::msaa::offscreen_pipelines_for_sample_count(
                        resources,
                        sample_count,
                    ),
            ...
            encode_scene_color_passes(
                ...
                SceneColorPasses {
                    ...
                    transmission_pipelines: resources
                        ...
                        .pipelines
                        .as_ref()
                        .map(super::super::pipeline::MeshPipelineSet::refs),
        ...
        let post_output = if post_enabled {
            ...
            if readback {
                super::super::readback::encode_copy_target_to_readback(
                    &mut encoder,
                    resources,
                    target,
                );
            ...
            (surface_view.as_ref(), resources.surface_pipeline.as_ref())
        {
            ...
            encode_scene_color_passes(
                ...
                SceneColorPasses {
                    ...
                    opaque_output_bind_group: resources
        ...
        if readback {
            super::super::readback::map_readback_to_frame(&self.device, resources, target, frame)?;
        }
        self.finalize_native_frame(
            target,
            NativeFrameResultInputs {
                reconfigure_after_present,
                reconfigurations,
                surface_acquire_retries,
