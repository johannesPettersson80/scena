...
fn fs_main(in: VertexOut) -> @location(0) vec4<f32> {
    ...
    if max_luma - min_luma < 16.0 / 255.0 {
        return finalize_post_output(center);
    }
    let dark_edge = center_luma - min_luma <= 1.0 / 255.0 && bright_neighbors >= 1u;
    let light_edge = max_luma - center_luma <= 1.0 / 255.0 && dark_neighbors >= 1u;
    if !dark_edge && !light_edge {
        return finalize_post_output(center);
    }
    return finalize_post_output(sum / 5.0);
}
