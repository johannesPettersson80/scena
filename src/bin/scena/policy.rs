...
pub(crate) fn push_allow_root(
    ...
    let value = args
        .get(index + 1)
        .ok_or_else(|| "--allow-root requires a directory path".to_owned())?;
    if value.is_empty() {
        return Err("--allow-root requires a non-empty directory path".to_owned());
    }
...
) -> Result<scena::RecipeBuildPolicy, String> {
...
pub(crate) fn run_recipe_policy_command(args: &[String]) -> Result<CliOutcome, String> {
