...
// T04 external anchor. The 256px reference is generated by scena itself, so
// comparing a live scena render to it proves determinism, not correctness. The
// Blender Cycles render is the only bundled image produced by an independent
// renderer; this file's `EXTERNAL_ANCHOR_*` constants bind the 256px reference
// to it automatically instead of relying solely on a human approval attesting
// `external_anchor_reviewed`.
const EXTERNAL_ANCHOR_PATH: &str =
    "tests/assets/gltf/khronos/WaterBottle/reference_blender_cycles_512.png";
const EXTERNAL_ANCHOR_SHA256: &str =
    "17db39248ce1966ae60c3b85d09491ebfb7f654777dc2d150a64db4e938a6883";
const REFERENCE_METADATA_PATH: &str =
    "tests/assets/gltf/khronos/WaterBottle/reference_metadata.toml";
/// Two renderers with different shading, tonemapping, and sampling never match
/// pixel-for-pixel. What must agree is *material classification*: which hue
/// family each region lands in. The bound is half the body/cap hue separation
/// the anchor itself exhibits (38 degrees), so a passing result can never be
/// produced by confusing the two materials.
const MAX_CROSS_RENDERER_HUE_DEGREES: f32 = 19.0;
...
/// T04 (`N7`): automated external anchor for the 256px CPU reference.
///
/// Before this test the 256px reference had no automated independent check —
/// promotion relied on a human ticking `external_anchor_reviewed`. Every other
/// Q01/Q11 assertion compares scena against scena, so all of them stay green if
/// the reference drifts to the wrong materials.
///
/// This binds the committed 256px reference to the bundled Blender Cycles
/// render, which no scena code path produces, on the one property two
/// independent PBR renderers must share: material hue classification.
#[test]
fn q11_cpu_reference_agrees_with_independent_blender_anchor_on_material_families() {
    let reference = read_reference_png();
    assert_eq!(
        sha256_hex(&reference.encoded),
        REFERENCE_SHA256,
        "the anchored image must be the committed 256px reference"
    );
    let anchor = read_rgba_png(EXTERNAL_ANCHOR_PATH);
    assert_eq!(
        sha256_hex(&anchor.encoded),
        EXTERNAL_ANCHOR_SHA256,
        "the external anchor must be the committed Blender Cycles render"
    );

    // The automated check and the human promotion path must reference the same
    // anchor bytes, otherwise this test could drift away from what a reviewer
    // attests to.
    let metadata =
        std::fs::read_to_string(REFERENCE_METADATA_PATH).expect("reference metadata reads");
    assert!(
        metadata.contains(&format!(
            "external_anchor_sha256 = \"{EXTERNAL_ANCHOR_SHA256}\""
        )),
        "reference_metadata.toml must record the same external anchor this test asserts against",
    );

    for material in material_samples() {
        let anchor_rgb = anchor.mean_rgb(material.anchor_x, material.anchor_y, 6);
        let reference_rgb = reference.mean_rgb(material.anchor_x / 2, material.anchor_y / 2, 3);
        let anchor_hue = hue_degrees(anchor_rgb);
        let reference_hue = hue_degrees(reference_rgb);

        assert!(
            material.contains_hue(anchor_hue),
            "{} anchor sample {anchor_rgb:?} has hue {anchor_hue:.1} deg, outside the declared \
             {} band {:?} — the external anchor no longer depicts the material it is named for",
            material.name,
            material.family,
            material.band,
        );
        assert!(
            material.contains_hue(reference_hue),
            "{} 256px reference sample {reference_rgb:?} has hue {reference_hue:.1} deg, outside \
             the declared {} band {:?} — the committed reference renders the wrong material",
            material.name,
            material.family,
            material.band,
        );
        let separation = circular_hue_distance(anchor_hue, reference_hue);
        assert!(
            separation <= MAX_CROSS_RENDERER_HUE_DEGREES,
            "{} disagrees between renderers by {separation:.1} deg (limit \
             {MAX_CROSS_RENDERER_HUE_DEGREES}): Blender {anchor_rgb:?} at {anchor_hue:.1} deg vs \
             scena 256px {reference_rgb:?} at {reference_hue:.1} deg",
            material.name,
        );
    }

    // The oracle must reject a wrong anchor, not merely accept the right one.
    // Rotating the anchor's channels preserves luminance and contrast — a
    // brightness- or contrast-based check would still pass — but moves every
    // material into a different hue family.
    let rotated = anchor.channel_rotated();
    let mut rejected = 0;
    for material in material_samples() {
        let rotated_rgb = rotated.mean_rgb(material.anchor_x, material.anchor_y, 6);
        if !material.contains_hue(hue_degrees(rotated_rgb)) {
            rejected += 1;
        }
    }
    assert_eq!(
        rejected,
        material_samples().len(),
        "a channel-rotated anchor must fail every material band; the hue oracle is not \
         discriminating if it accepts one",
    );

    // The failure this row exists to prevent: a reference promoted from a build
    // that renders the wrong material. Byte-comparing such a reference against a
    // live render of the same wrong build passes, and so does the
    // byte-determinism proof — only the external anchor rejects it. Prove that
    // on a real render, not a post-hoc pixel edit.
    let wrong_material = ReferenceImage {
        encoded: Vec::new(),
        rgba: render_wrong_material_scene(),
        width: SIZE,
        height: SIZE,
    };
    let mut wrong_material_rejected = 0;
    for material in material_samples() {
        let sample = wrong_material.mean_rgb(material.anchor_x / 2, material.anchor_y / 2, 3);
        if !material.contains_hue(hue_degrees(sample)) {
            wrong_material_rejected += 1;
        }
    }
    assert_eq!(
        wrong_material_rejected,
        material_samples().len(),
        "a reference promoted from a wrong-material build must fail every material band, \
         otherwise this anchor cannot catch the regeneration failure it exists for",
    );
}
...
struct MaterialSample {
    name: &'static str,
    family: &'static str,
    /// Sample center in the 512px anchor; the 256px reference is sampled at
    /// half these coordinates because both renders share the same framing.
    anchor_x: usize,
    anchor_y: usize,
    /// Inclusive hue band in degrees, wrapping through 0 when `start > end`.
    band: (f32, f32),
}

impl MaterialSample {
    fn contains_hue(&self, hue: f32) -> bool {
...
fn material_samples() -> [MaterialSample; 2] {
    [
        MaterialSample {
            name: "bottle body",
            family: "olive/yellow",
...
fn hue_degrees(rgb: [f32; 3]) -> f32 {
    let [r, g, b] = rgb.map(|channel| channel / 255.0);
...
fn read_rgba_png(path: &str) -> ReferenceImage {
    let encoded = std::fs::read(path).unwrap_or_else(|error| panic!("{path} reads: {error}"));
    let decoder = png::Decoder::new(std::io::Cursor::new(&encoded));
    let mut reader = decoder
        .read_info()
        .unwrap_or_else(|error| panic!("{path} header reads: {error}"));
    assert_eq!(reader.info().bit_depth, png::BitDepth::Eight);
    let mut rgba = vec![
        0;
        reader
            .output_buffer_size()
            .unwrap_or_else(|| panic!("{path} output size is known"))
    ];
    let info = reader
        .next_frame(&mut rgba)
        .unwrap_or_else(|error| panic!("{path} payload reads: {error}"));
    let channels = match info.color_type {
        png::ColorType::Rgba => 4,
        png::ColorType::Rgb => 3,
        other => panic!("{path} must be RGB or RGBA, found {other:?}"),
    };
    if channels == 3 {
        rgba = rgba[..info.buffer_size()]
            .chunks_exact(3)
...
impl ReferenceImage {
    /// Mean RGB over the `(2 * radius + 1)`-square patch centered on `(x, y)`.
    /// Averaging suppresses single-pixel shading noise so the hue reading
    /// describes the material, not one sample.
    fn mean_rgb(&self, x: usize, y: usize, radius: usize) -> [f32; 3] {
        let mut totals = [0.0_f64; 3];
        let mut samples = 0.0_f64;
        let min_y = y.saturating_sub(radius);
        let max_y = (y + radius).min(self.height as usize - 1);
        let min_x = x.saturating_sub(radius);
        let max_x = (x + radius).min(self.width as usize - 1);
        for row in min_y..=max_y {
            for column in min_x..=max_x {
                let offset = (row * self.width as usize + column) * 4;
                for (channel, total) in totals.iter_mut().enumerate() {
                    *total += f64::from(self.rgba[offset + channel]);
                }
                samples += 1.0;
            }
        }
        totals.map(|total| (total / samples) as f32)
    }

    fn channel_rotated(&self) -> Self {
        let rgba = self
            .rgba
            .chunks_exact(4)
            .flat_map(|pixel| [pixel[2], pixel[0], pixel[1], pixel[3]])
            .collect();
        Self {
            encoded: Vec::new(),
            rgba,
