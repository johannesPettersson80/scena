...
fn close_camera_near_clip_matches_cpu_and_gpu_rendered_output() {
    if std::env::var_os("SCENA_RUN_UNSTABLE_HEADLESS_GPU_RELEASE_TESTS").is_none() {
        let artifact_dir = std::path::Path::new("target/gate-artifacts/c13-cpu-depth-clipping");
        std::fs::create_dir_all(artifact_dir).expect("C13 diagnostic artifact directory creates");
        std::fs::write(
            artifact_dir.join("gpu-parity-unavailable.json"),
            concat!(
                "{\n",
                "  \"schema\": \"scena.c13_cpu_depth_clipping_gpu.v1\",\n",
                "  \"release_evidence\": false,\n",
                "  \"reason\": \"explicit_gpu_parity_lane_not_requested\"\n",
                "}\n"
            ),
        )
        .expect("C13 fail-closed diagnostic artifact writes");
        return;
    }
