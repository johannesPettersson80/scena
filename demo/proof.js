import init, {
  ...
  tick,
} from "./proof/pkg/scena.js?v=1.10.3-proof-19d17fc36c12";
...
function updateCodePanel() {
  if (activeAsset.code === "v14-presets") {
    ...
    codeSubtitle.textContent = "scena 1.10.3 — pick a name, not a number";
...
async function start() {
  ...
  await init({
    module_or_path: new URL(
      "./proof/pkg/scena_bg.wasm?v=1.10.3-proof-19d17fc36c12",
      import.meta.url,
    ),
  });
  wireDragDrop();
  ...
  const sample =
    SAMPLES.find((entry) => entry.id === sampleParam) ||
    SAMPLES.find((entry) => entry.id === "material-presets") ||
    SAMPLES[0];
