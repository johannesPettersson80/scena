...
let orbit = { yaw: -0.62, pitch: 0.28, distance: 2.0 };
...
function beginPhase(label) {
  ...
  setPhase(label);
  console.info(`[scena-demo] ${activeAsset.label}: ${label}`);
}
...
function updateCodePanel() {
  ...
  codeSubtitle.textContent = activeAsset.path;
...
async function loadSample(sample) {
  ...
  updateActiveButton();
  updateCodePanel();
  beginPhase("fetching sample");
  try {
    const response = await fetch(sample.path);
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    const bytes = new Uint8Array(await response.arrayBuffer());
    console.info(`[scena-demo] ${sample.label}: fetched ${bytes.byteLength} bytes`);
...
async function loadAndAttach(bytes, asset, byteLength) {
  attached = false;
  activeAsset = asset;
  orbit = { yaw: -0.62, pitch: 0.28, distance: 2.0 };
  updateCodePanel();
