...
pub(super) fn check_material_presets_failing_baseline(root: &Path, findings: &mut Vec<Finding>) {
    ...
    let minimum_margin = artifact["minimum_failure_margin_delta_e2000"]
        .as_f64()
        .unwrap_or(0.0);
    if threshold > 4.0 || minimum_margin < 1.0 {
        findings.push(Finding::new(
            "HONEST-MATERIAL-PRESETS",
            format!(
                "{ROUND_E_FAILING_BASELINE} must keep threshold <= 4.0 and failure margin >= 1.0"
            ),
        ));
    }
    ...
    let meaningful_failures = failed
        ...
        .count();
    if meaningful_failures < 3 {
        findings.push(Finding::new(
            "HONEST-MATERIAL-PRESETS",
            format!("{ROUND_E_FAILING_BASELINE} must show at least three meaningful failures"),
        ));
    }
