...
reports, renderer statistics, and release evidence so applications can
distinguish unavailable features from renderer failures.

## Compatibility Notes

- `Renderer::prepare` is the synchronization point for scene, material,
  environment, target, and renderer-setting changes.
