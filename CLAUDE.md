...
| `SCENA_BROWSER_DUMPIO` | Set to `1` to stream Chromium stdout and stderr while diagnosing a browser probe. | disabled |
| `SCENA_BROWSER_GPU_DIAGNOSTICS` | Set to `1` to emit structured adapter, pipeline-allocation, and browser readback diagnostics. | disabled |
...
| `SCENA_BROWSER_SKIP_VIEWER_ELEMENT` | Set to `1` to skip the viewer-element branch during a focused renderer probe. | full M6 probe |
