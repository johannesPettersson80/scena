...
|---|---|---|
| `SCENA_USE_GPU` | Legacy metadata-only bit written into the WaterBottle renderer companion. It does not select the GPU lane. | unset → metadata records false |
...
| `SCENA_REFERENCE_DIFF` | Diagnostic-only WaterBottle GPU reference comparison: at least 95% of pixels must be within RGB Chebyshev distance 16 of `reference_512.png`. No required CI or release workflow sets this flag, so the GPU lane does not claim a golden diff. | unset → reference comparison skipped; nonblack, histogram, and region assertions still run |
